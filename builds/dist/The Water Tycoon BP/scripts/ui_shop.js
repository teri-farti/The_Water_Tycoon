...
let isGuiOpen = false; // Защита от дубликатов

world.beforeEvents.itemUseOn.subscribe(ev => {
    const { block, source: player } = ev;
    if (block.typeId !== "water_tycoon:worker_shop" || isGuiOpen) return;

    isGuiOpen = true; // Закрываем замок
    system.run(() => {
        openShop(player);
        // Открываем замок через полсекунды
        system.runTimeout(() => { isGuiOpen = false; }, 10);
    });
});